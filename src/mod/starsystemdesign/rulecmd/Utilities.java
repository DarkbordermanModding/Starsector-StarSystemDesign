...
public class Utilities
{
    ...
    public static SectorEntityToken getClosetEntity(SectorEntityToken source, double radius, String tag){
        /**
        ...
        * @param  tag    filter only entities with specific tag, null if not specified
        ...
        */
        ...
        for(SectorEntityToken token: tokens){
            if(token == source) continue;  // Yourself does not count
            else if(getDistanceBetweenTokens(token, source) < radius)
            {
                radius = getDistanceBetweenTokens(token, source);
                if(tag != null){
                    if(token.hasTag(tag)) closet = token;
                } else closet = token;
            }
        }
        return closet;
